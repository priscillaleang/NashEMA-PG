...
class FeatureExtractor(nnx.Module):
    """Feature extractor for perfect recall Leduc poker observations."""
    
    def __init__(self, key: chex.PRNGKey):
        rngs = nnx.Rngs(key)
        self.mlp = nnx.Sequential(
            nnx.Linear(in_features=49, out_features=MLP_DIM, rngs=rngs),
            nnx.relu,
...
class LeducPokerAgent(BaseAgent):
    """Leduc poker agent with perfect recall and specialized policy/critic networks."""
    ...
    def __init__(self, key: chex.PRNGKey):
        ...
        self._policy_head = nnx.Linear(in_features=MLP_DIM, out_features=3, rngs=rngs)

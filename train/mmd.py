"""
MMD https://arxiv.org/abs/2206.05825
In original paper we didn't find open source implementation of MMD in RL framework, the only hint is in page 7
"We implemented MMD as a modification of RLlib's PPO implementation by changing the adapative forward KL
regularization to a reverse KL regularization."
However, we did try this but found out performance didn't align with what they reported, therefore, we decide
to use the same adaptation shceme we use for MMD for fair comparison, which give them a better performance.
Another point is they didn't specify what is there magnect agent, from most of the paper context, we assume is
a random agent, with that said, it should take a grant of sault as we can't be sure we use the same implementation
...
from agents import create_agent, BaseAgent, RandomAgent
...
def main(config: DictConfig):
    ...
    # setup learner state
    key, learner_key, random_agent_key = jax.random.split(key, 3)
    random_agent = RandomAgent(random_agent_key, env=env)
    learner_state = LearnerState(
        key=learner_key,
        env_state=env_state,
        ...
        mag_agent=random_agent,
    )

    # setup logger
    logger = create_logger(config)
    logger.log_config(config)
    assert config.algorithm.num_update % config.logging.log_interval == 0, "log_interval must be a divisible by num_update"
